...
import "./Grid.scss";
import {ComponentClass, FunctionComponent, useCallback, useEffect, useMemo, useRef, useState, memo} from "react";
...
import {fetchForecastData, setForecastData} from "features/budgetmanager/operations/forecastSlice";
...
const SUIGrid = (props: TableGridProps) => {
	...
	const [ callForecastApi, setCallForecastApi ] = useState<any>(false);
  ...
  useEffect(()=> {
		...
		if(props.updatedObj) {
			...
			if(obj) {
				...
				const payload = {
					...
					estimatedStart: convertDateToDisplayFormat(payloadObj.estimatedStart),
					estimatedEnd: convertDateToDisplayFormat(payloadObj.estimatedEnd),
	...
	if(rowModelType == 'serverSide') {
		...
		// defaultProps.paginationPageSize = 50;
	} else {
	...
	useEffect(() => {
		gridRef?.current?.api?.setColumnDefs(effHeaders);
	...
	useEffect(() => {
		if(liveData) {
			...
			if('add' in liveData) {
				...
				dispatch(setOriginalGridApiData([ ...originalGridApiData, ...liveData.add ]));

				setTimeout(() => {
					gridRef.current!?.api?.forEachNode((rowNode: any, index: any) => {
						if(rowNode.data) {
							if(rowNode.data?.id == liveData.add[ 0 ]?.id) {
								const newlyAddedRec = gridRef.current!?.api?.getDisplayedRowAtIndex(rowNode.rowIndex)!;
								gridRef.current!?.api?.flashCells({rowNodes: [ newlyAddedRec ]});
							}
						}
					});
				}, 500);
			...
			if('update' in liveData) {
				...
				gridRef.current!?.api?.forEachNode((row: any) => {
					if(row?.data) {
						if(row?.data?.id == liveData.update[ 0 ]?.id) {
							...
							updatedRec = gridRef.current!?.api?.getDisplayedRowAtIndex(row.rowIndex)!;
						}
					}
				});
				...
				setTimeout(() => {
					...
					if(diffKey.length) {
						...
						gridRef.current!?.api?.flashCells({rowNodes: [ updatedRec ], columns: [ 'division', ...diffKey ]});
					}
				}, 2000);
	...
	return (
		<div style={gridStyle} className="ag-theme-alpine sui-grid grid-copy">
			...
			<AgGridReact
				...
				// onRowClicked={(e) => onRowClicked(e, gridRef)}
				onRowDoubleClicked={(e) => onRowDoubleClicked(e, gridRef)}
				...
				// onGridReady={() => { setAGGridRef(gridRef); console.log('table ref', gridRef) }}
				// {...rest}
				...
				overlayNoRowsTemplate={`<div class="no-rows-msg"><span class="common-icon-No-Item-Available"></span><div class="empty-rows-mark">${emptyMsg}</div>${nowRowsMsg ? nowRowsMsg : ""
					}</div>`}
