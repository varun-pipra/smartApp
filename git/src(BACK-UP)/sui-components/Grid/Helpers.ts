...
export abstract class Helpers {
		...
		switch(type) {
			...
			case "currency":
				...
				cfg['cellRenderer'] = (params: any) => {
					return params.value ? appCfg.currencySymbol + " " + params?.value?.toLocaleString("en-US") : '';
				};
				break;
			case "coloredNumeric":
				cfg['type'] = "rightAligned";
			...
			case "coloredCurrency":
				...
				cfg['cellRenderer'] = (params: any) => {
					...
					return params.value ? `${appCfg.currencySymbol} ${params?.value?.toLocaleString("en-US")} ${op}` : '';
				};
				cfg['cellStyle'] = (params: any) => {
					if(params.value < 0) {
						return {color: 'red'};
	...
	public static generatePinnedBottomData(gridApi: any, gridColumnApi: any, pinnedBottomRowConfig: any) {

		// generate a row-data with null values
		let result: any = {};

		gridColumnApi.getAllGridColumns().forEach((item: any) => {
			result[item.colId] = pinnedBottomRowConfig.displayFields[item.colId] || null;
		});

		return this.calculatePinnedBottomData(gridApi, result, pinnedBottomRowConfig);
	}

	public static calculatePinnedBottomData(gridApi: any, target: any, pinnedBottomRowConfig: any) {
		//console.log(target);
