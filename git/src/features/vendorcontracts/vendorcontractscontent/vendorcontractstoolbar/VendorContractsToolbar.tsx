...
import { getVendorContractsList, setGridData, setActiveMainGridGroupKey, setActiveMainGridFilters, setSelectedRows, setMainGridSearchText, setActiveMainGridDefaultFilters } from "../../stores/gridSlice";
...
import { activateContract, lockAndPostContract } from '../../stores/VCButtonActionsAPI';
...
const VendorContractsToolbar = (props: any) => {
	...
	const { selectedRows } = useAppSelector((state) => state.vendorContractsGrid);
	...
	return <Stack direction='row' className='toolbar-root-container-vendor-contracts'>
		...
		/>}
		{
			showAlertForPendingCompliance?.show && <SUIAlert
				...
				title={'Confirmation'}
				onAction={(e: any, type: string) => {type == 'yes' ? handlePostContract(true) :
					setShowAlertForPendingCompliance({ show: false, message: '' });}}
				showActions={true}
				modelWidth={'610px'}
			/>
