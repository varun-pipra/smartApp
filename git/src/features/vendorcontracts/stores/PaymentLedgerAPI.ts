...
export const fetchPaymentLedgerList = async (appInfo: any, contractId:any) => {
	...
	if (!isLocalhost) {
		if(contractId) {
			const response = await payAppsRequest(appInfo, `?vendorcontractid=${contractId}`, {});
...
export const fetchVendorContractChangeEvents = async (appInfo: any, contractId:string) => {
	...
	if (!isLocalhost) {
		if(contractId) {
			response = await fetch(`${appInfo?.hostUrl}/enterprisedesktop/api/v2/projects/${appInfo.uniqueId}/finance/vendorcontracts/${contractId}/changeevents?sessionId=${appInfo?.sessionId}`);
			if (!response?.ok) {
