...
export const fetchPaymentLedgerList = async (appInfo: any, contractId: any) => {
	...
	if (!isLocalhost) {
		if (contractId) {
			...
			response = await fetch(`${appInfo?.hostUrl}/EnterpriseDesktop/api/v2/projects/${appInfo?.uniqueId}/finance/clientpayapps?clientcontractid=${contractId}&offset=0&limit=20000&sessionId=${appInfo?.sessionId}`);
			if (!response?.ok) {
...
export const fetchClientContractChangeEvents = async (appInfo: any, contractId:string) => {
	...
	if (!isLocalhost) {
		if (contractId) {
			...
			response = await fetch(`${appInfo?.hostUrl}/enterprisedesktop/api/v2/projects/${appInfo.uniqueId}/finance/clientcontracts/${contractId}/changeevents?offset=0&limit=20000&sessionId=${appInfo?.sessionId}`);
			if (!response?.ok) {
				const message = `API Request Error (${moduleName}): ${response?.status}`;
				// return response							
				throw new Error(message);
