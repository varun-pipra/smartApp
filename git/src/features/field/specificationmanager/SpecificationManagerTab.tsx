...
interface SpecManagerWindowProps {
  activeTab?: String;
};
const SpecificationManagerTab = ({activeTab, ...props} : SpecManagerWindowProps) => {
  ...
  const onRowSelection = useCallback((e: any) => {
    let ids: any = [];
      ...
      const filterVal =
        ...
        (!_.isEmpty(selectedFilters) &&
          (_.isEmpty(selectedFilters.division) ||
            ...
            selectedFilters.division?.indexOf(
              `${item.division.number} - ${item.division.text}`
            ) > -1) &&
...
export default memo(SpecificationManagerTab);
