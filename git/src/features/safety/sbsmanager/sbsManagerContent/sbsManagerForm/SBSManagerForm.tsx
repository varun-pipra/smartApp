...
const SBSManagerForm = (props: any) => {
	...
	const handleAdd = () => {
		...
		data.startDate = formData?.startDate === '' ? null : formData.startDate;
		data.endDate = formData?.endDate === '' ? null : formData.endDate;
		const payload = {
			...data,
			uniqueID: uuidForSbs,
		};
		AddSbsManagerForm(payload)
			.then((res: any) => {
				...
				if(res?.error?.message) {
					dispatch(setToast(res?.error?.message));
				  } else {
			...
			.catch((err: any) => {
				...
				if(err?.error?.message) {
					dispatch(setToast(err?.error?.message));
				  };
			});
	};
