...
const ClientPayApplicationsLID = ({data, ...props}: IQGridWindowDetailProps) => {
	...
	const disableBlockchainActionButtons = (blockchainEnabled && blockchainStates.indexOf(clientPayAppLineItem?.blockChainStatus) === -1);

	React.useEffect(() => {
		setContractorResponse({...contractorResponse, show: ["Rejected"]?.includes(clientPayAppLineItem?.status), type: 3, data: clientPayAppLineItem?.clientAuthorization?.rejection ? clientPayAppLineItem?.clientAuthorization : clientPayAppLineItem?.gcAuthorization});
