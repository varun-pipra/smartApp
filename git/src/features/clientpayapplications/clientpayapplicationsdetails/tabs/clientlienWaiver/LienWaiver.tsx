...
const LienWaiver = (props: LienWaiverProps) => {
	...
	React.useEffect(() => {
		...
		else { setSignature(selectedRecord?.clientAuthorization?.signature); setUserDetails(selectedRecord?.clientAuthorization?.displayName) }
	}, [selectedRecord, appInfo])

	// React.useEffect(() => {
	// if(signaturePadRef?.current && (selectedRecord?.gcAuthorization?.signature || selectedRecord?.clientAuthorization?.signature)) {
	//   isUserGCForCPA(appInfo) ?  signaturePadRef?.current?.fromDataURL(selectedRecord?.gcAuthorization?.signature) : signaturePadRef?.current?.fromDataURL(selectedRecord?.clientAuthorization?.signature)	
	// } else {
		...
		if (isUserGCForCPA(appInfo)) {
			if (["AutoGeneratedWaitingForBothParties", 'Draft']?.includes(selectedRecord?.status)) {
				if (selectedRecord?.gcAuthorization?.signature) {
					setChecked(true); setShowAmount(true)
				}
			}
		}
		else {
			{
				if (["AutoGeneratedWaitingForBothParties", 'Draft']?.includes(selectedRecord?.status)) {
					if (selectedRecord?.clientAuthorization?.signature) {
						setChecked(true); setShowAmount(true)
					}
				}
			}
	...
	React.useEffect(() => {
		let isValidPayment: boolean = ['DownPayment']?.includes(selectedRecord?.type) ? true : false;
		selectedRecord?.billingSchedule?.payments?.find((payment: any) => {
			if (payment?.status == 'SelectedForPayment') isValidPayment = true;
		})
		console.log("signaturePadRef", signature, checked, filesToView, showAmount, selectedRecord?.scheduleOfValues, isValidPayment);
	...
	const handleChange = (e: any) => {
		console.log("eee", e)
		if (checked) {
			const isValid = isUserGCForCPA(appInfo) ? selectedRecord?.gcAuthorization?.signature != null ? true : false : selectedRecord?.clientAuthorization?.signature != null ? true : false
			isValid && patchClientPayAppDetails(appInfo, { signedOn: null, signature: null }, selectedRecord?.id, (response: any) => {
	...
	const saveContractAttachments = (contracts: any) => {
		...
		patchClientPayAppDetails(appInfo, payload, selectedRecord?.id, (response: any) => {
			dispatch(setSelectedRecord(response));
		})
	};
	...
	const closeFileViewer = () => {
		setFilesToView([]);
		patchClientPayAppDetails(appInfo, { stream: null, signedOn: null, signature: null }, selectedRecord?.id, (response: any) => {
			// dispatch(setSelectedRecord(response))
		})
		setShowAmount(false);
		setChecked(false);
	...
	const clearSignature = () => {
		...
		patchClientPayAppDetails(appInfo, { signedOn: null, signature: null }, selectedRecord?.id, (response: any) => {
			// dispatch(setSelectedRecord(response))
		})
		setSignedOn(null);
	...
	const getGCForm = (gc: any, sc: any) => {
		return <div className="awaiting-stat">
			<span className="awaiting-stat-vendor-sign">{getSignature(sc?.signature, sc?.displayName, sc?.signedOn, 'Client Signature')}</span>
	...
	const getSCForm = (gc: any, sc: any) => {
		return <div className="awaiting-stat">
			...
			<span className="awaiting-stat-General-sign">{getSignaturePad(sc?.signature, sc?.displayName, sc?.signedOn, 'Client Signature')}</span>
		</div>
	};

	const getReadOnlySign = () => {
		const data = isUserGCForCPA(appInfo) ? selectedRecord?.gcAuthorization : selectedRecord?.clientAuthorization;
		return <div className="sumitted-stat">{getSignature(data?.signature, data?.displayName, data?.signedOn, isUserGCForCPA(appInfo) ? 'General Contract Signature' : 'Client Signature')}</div>
	}

	const getGCAndSCSigns = (gc: any, sc: any) => {
		const data = isUserGCForCPA(appInfo) ? selectedRecord?.gcAuthorization : selectedRecord?.clientAuthorization;
		return <div className="auto-gen-stat">{getSignaturePad(data?.signature, data?.displayName, data?.signedOn, isUserGCForCPA(appInfo) ? 'General Contract Signature' : 'Client Signature')}</div>
	}

	const getReadOnlySigns = () => {
		const gc = selectedRecord?.gcAuthorization
		const sc = selectedRecord?.clientAuthorization
	...
	const handleOnEnd = () => {
		console.log("handleOnEnd", signaturePadRef, signaturePadRef.current.isEmpty())
		if (signaturePadRef?.current && !signaturePadRef.current.isEmpty()) {
			const sign = signaturePadRef?.current?.getTrimmedCanvas().toDataURL("image/png")
			setSignature(sign);
			dispatch(setSignatureToAuthorize(sign));
			selectedRecord?.status != 'AwaitingAcceptance' && patchClientPayAppDetails(appInfo, { signature: sign, signedOn: new Date() }, selectedRecord?.id, (response: any) => {
				dispatch(setSelectedRecord(response))
			})
			...
			<div className="vendor-lienwaiver-secondSection">
				...
				<div className="secondsection_Content">
					<p className="second_content2">
						<Checkbox checked={checked} onChange={handleChange} disabled={["AutoGeneratedWaitingForBothParties", "Draft", "AwaitingAcceptance"]?.includes(selectedRecord?.status) ? false : true} />
				...
				</div>
				{
					selectedRecord?.status == 'Draft' ? <div className="signature_section">
						<span className="common-icon-signature"></span>
						<label>{isUserGCForCPA(appInfo) ? 'General Contractor Signature' : 'Client Signature'}</label>
						<div className="signature" style={{ width: '100%' }}>
							<SignatureCanvas
								ref={signaturePadRef}
								penColor="black"
						...
						: selectedRecord?.status == 'AwaitingAcceptance' ?
							isUserGCForCPA(appInfo) ? <div>{getGCForm(selectedRecord?.gcAuthorization, selectedRecord?.clientAuthorization)}</div> : <div>{getSCForm(selectedRecord?.gcAuthorization, selectedRecord?.clientAuthorization)}</div>
							: selectedRecord?.status == 'SubmittedWaitingForOtherParty' ? getReadOnlySign() : selectedRecord?.status == 'AutoGeneratedWaitingForBothParties' ? getGCAndSCSigns(selectedRecord?.gcAuthorization, selectedRecord?.clientAuthorization) : getReadOnlySigns()
				}
