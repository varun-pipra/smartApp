...
const ProjectTeamApplicationsLID = ({data, iframeEventData, ...props}: IQGridWindowDetailProps) => {
	...
	const helpTabEnum: any = {
		userDetails: 'DETAILS',
		userBadge: 'BADGE',
		safetyCredentials: 'CREDENTIALS',
	...
	React.useEffect(() => {
		if(helpTabEnum[activeTab] || (helpTabEnum[props?.defaultTabId] && !isInitialRender?.current)) {
		isInitialRender.current = true;
		const body = {iframeId: iframeID, appType: appType, tabName: helpTabEnum[activeTab] || helpTabEnum[props?.defaultTabId], isFromHelpIcon: isFromHelpIcon};
		postMessage({
			event: "help",
			body: body,
		});
	...
	const handleHelp = (e: any) => {
		...
		const body = {iframeId: iframeID, appType: appType, tabName: helpTabEnum[activeTab] || helpTabEnum[props?.defaultTabId], isFromHelpIcon: true};
		postMessage({
			event: "help",
			body: body,
		});
	...
	const handleSaveUserDetails = () => {
		...
		if(activeTab == 'certifications') {
			...
			const Payload = {
				...
				hourlyRate : typeof userDetails?.hourlyRate === 'string' ? parseFloat(userDetails?.hourlyRate?.replace(/,/g, '')) : parseFloat(userDetails?.hourlyRate)
			};
