...
export const ReferenceFiles = ({ iFrameId, appType, readOnly }: any) => {
	...
	const { specSelectedRecInAddSpecDlg, markupsByPageForBid, selectedRecord } = useAppSelector((state) => state.bidManager);
	...
	useEffect(() => {
		...
		const value = selectedRecord.budgetItems.map((budget: any) => budget?.division);
