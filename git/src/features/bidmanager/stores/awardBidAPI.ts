...
export const awardBid = async (appInfo: any, packageId: any, bidderUniqueID: any, createContract?: boolean) => {
	if(!isLocalhost) {
		let response = await fetch(`${appInfo?.hostUrl}/EnterpriseDesktop/api/v2/projects/${appInfo?.uniqueId}/bids/${packageId}/bidders/${bidderUniqueID}/awardbid?sessionId=${appInfo?.sessionId}&createcontract=${createContract}`, {
			method: 'POST',
