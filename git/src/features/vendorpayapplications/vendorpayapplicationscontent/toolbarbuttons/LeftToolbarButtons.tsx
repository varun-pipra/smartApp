...
const VendorPayAppToolbarLeftButtons = () => {
	...
	return <>
		...
		{isUserGCForVPA(appInfo) && <IconButton className='text-btn-cls' aria-label='Mark Payment Sent' disabled={!isSingleSelected && disablePaymentSent} onClick={() => {setPaymentSentClick(true);}}>
			<span className='common-icon-tickmark'></span>
			Mark Payment Sent
		</IconButton>}

		{!isUserGCForVPA(appInfo) && <IconButton className='text-btn-cls' disabled={!isSingleSelected && disablePaymentReceive} aria-label='Mark Payment Received' onClick={() => {
