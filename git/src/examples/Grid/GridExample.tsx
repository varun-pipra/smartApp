...
const GridExample = (props: any) => {
	...
	const [columnDefs, setColumnDefs] = React.useState([
		...
		{ headerName: "Budget ID/CBS", field: "name", type: "showCount" },
	...
	const [rowData] = React.useState([
		{
			...
			"name": ['General Requirement', 'GeneralContractor', 'Owner Cost', 'Owner Cost'],
	...
	const autoGroupColumnDef = {
		...
		// valueFormatter: (params: any) => customCellRendererClass(params),
		pinned: "left",
		sort: "asc",
		width: 550,
		resizable: true,
	...
	const customValueGetter = (params: any) => {
		...
		return params.data?.costCode + '-' + params.data?.costType;
	}
					...
					<IQTooltip
						title={
							<Stack direction='row'>
								...
								<p style={{ marginTop: '5px' }}> Cost Code Is Mismatched</p>
							</Stack>}
						placement={"bottom"} arrow={true}
					>
						<WarningAmberIcon fontSize={primaryIconSize} style={{ color: 'red', position: 'absolute', left: '2%', marginTop: '10px' }} />
					</IQTooltip>
					<span className="ag-costcodegroup" style={{ textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden' }}>{params.data?.costCode + '-' + params.data?.costType} </span>
				</>
					...
					<>
						<IQTooltip
							title={<Stack direction='column'>
								<Stack direction='row'>
									<WarningAmberIcon fontSize={primaryIconSize} style={{ color: 'red' }} />
									<p style={{ marginTop: '5px' }}>Your Remaining Balance is Negative</p>
								</Stack>
								<Stack direction='row' mt={-1}>
									<WarningAmberIcon fontSize={primaryIconSize} style={{ color: '#f4e39f' }} />
									<p style={{ marginTop: '5px' }}>Your Forecasted Remaining Balance is Negative</p>
								</Stack>
							</Stack>}
							placement={"bottom"} arrow={true}
						>
