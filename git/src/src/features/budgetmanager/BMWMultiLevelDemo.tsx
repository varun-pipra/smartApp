...
const BMWMultiLevelDemo = (props: any) => {
	...
	const [isMaximize, setIsMaximize] = React.useState(appInfo?.fullScreen);
	const [isInline, setInline] = React.useState(false);
	const [isMaxByDefault, setMaxByDefault] = React.useState(false);
	...
	React.useEffect(() => {
		...
		if(search) {
			...
			setIsFullView(params?.get('inlineModule') === 'true');
		}
	}, [location]);
	...
	React.useEffect(() => {
		if(appInfo) {
			...
			setIsMaximize(appInfo?.fullScreen);
		}
	}, [appInfo]);
	...
	const optionalTools = <React.Fragment>{
		<>
...
export default BMWMultiLevelDemo;